...
export const CustomSelect = ({ options, value, onChange, placeholder }: Props) => {
  ...
  return (
    <div className="relative" ref={wrapperRef}>
      ...
      <div
        onClick={() => setOpen((p) => !p)}
        className="flex items-center justify-between w-full py-3 text-sm transition-colors duration-200 bg-transparent border-b cursor-pointer border-zinc-200 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100 hover:border-zinc-400 dark:hover:border-zinc-500"
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.ul
            ...
            className="absolute left-0 z-20 w-full mt-1 overflow-hidden bg-white border rounded-lg shadow-sm dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800"
          >
            {options.map((option) => {
